...
type Transaction = {
  ...
  date: string; department: string; paybackExpected: boolean; paidBack: boolean; givenBy?: string;
...
export default function Dashboard() {
  ...
  const [transactions, setTransactions] = useState<Transaction[]>([]);      // display list (last 50)
  const [allTransactions, setAllTransactions] = useState<Transaction[]>([]); // analytics (no limit)
  const [owings, setOwings] = useState<Owing[]>([]);
  const [categories, setCategories] = useState<{ id: string; name: string }[]>([]);
  const [departments, setDepartments] = useState<Dept[]>([]);
  const [activeDept, setActiveDept] = useState("ALL");
  const [activeTab, setActiveTab] = useState<"accounts" | "transactions" | "owing">("accounts");
  ...
  const fetchData = useCallback(async () => {
    ...
    const sep = deptParam ? "&" : "?";
    const [accs, txns, allTxns, ows, cats] = await Promise.all([
      fetch(`/api/accounts${deptParam}`).then((r) => r.json()),
      fetch(`/api/transactions${deptParam}${sep}limit=50`).then((r) => r.json()),
    ...
    <div className="min-h-screen bg-[#FAF9F6]">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        {loading ? (
          ...
          <>
            {/* mobile tab switcher */}
            <div className="flex lg:hidden gap-1 bg-white border border-[#EDE8DF] p-1 rounded-xl shadow-sm">
              {(["accounts", "transactions", "owing"] as const).map((t) => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
              ...
              <div className={`lg:col-span-3 space-y-4 ${activeTab !== "accounts" ? "hidden lg:block" : ""}`}>
                ...
                {accounts.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {accounts.map((a) => <AccountCard key={a.id} account={a} onUpdated={fetchData} departments={departments} />)}
                  </div>
                )}
                ...
                {pendingPaybacks.length > 0 && (
                  <div>
                    ...
                    <div className="bg-white rounded-2xl border border-[#F5D99B] shadow-sm divide-y divide-[#FEF9EE] px-3">
                      {pendingPaybacks.map((t) => <TransactionRow key={t.id} txn={t} onUpdated={fetchData} accounts={accounts} categories={categories} departments={departments} />)}
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className={`lg:col-span-2 space-y-5 ${activeTab === "accounts" ? "hidden lg:block" : ""}`}>
                ...
                <div className={activeTab === "owing" ? "hidden lg:block" : ""}>
                  <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="bg-white rounded-2xl border border-[#EDE8DF] shadow-sm overflow-hidden">
                    {transactions.length === 0 ? (
                      ...
                      <div className="px-3">
                        {transactions.map((t) => <TransactionRow key={t.id} txn={t} onUpdated={fetchData} accounts={accounts} categories={categories} departments={departments} />)}
                      </div>
                    )}
                  </div>
                </div>

                <div className={`bg-white rounded-2xl border border-[#EDE8DF] shadow-sm p-4 ${activeTab === "transactions" ? "hidden lg:block" : ""}`}>
